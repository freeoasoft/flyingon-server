...
exports.init = settings => {
    ...
    if (settings.cache === 'redis' && settings.redis)
    {
...
//缓存观测插件
exports.watch = async (context, next) => {

    let key = context.paths[0];
    let value;
    ...
    if (key)
    {
        if (key === '-')
        {
            if (key = context.paths[1])
            {
                value = await redis.del(key);
            }
            else
            {
                value = await redis.flushall();
            }
        }
        else
        {
